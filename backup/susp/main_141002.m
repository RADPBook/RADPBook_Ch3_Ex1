...
% W is the weights of V, such that V = w'*phi(x)
% and therefore 
% \partial V / \partial x = w'*dphi(x)'


% reset w for iterations
w   =   [0 0 0 0 2*R/B(2)*(-k(1)) zeros(1,ll-5)]' ;


%%
%---------Solve for the initial cost---------------------------------------
[tcost,ycost] = ode45(@(t,X) LP_model_calc(t,X,A,B,kBn/mB,Q,R,k,w), ...
	            [0,100], [x0;zeros(4*4+4+1,1)]);
J =  ycost(end,end);
clear tcost ycost;
J1_save = [J];
J2_save = [J];
        ...
        for x2 = -1.5:.5:1.5
            for x3 = -1.5:.5:1.5
                for x4 = -2:0.5:2
                    ...
                    X = [X;(dP*(A*[x1;x2;x3;x4]+[0;kBn/mB*x3^3;0; ...
                          -kBn/mB*x3^3]+B*u))'];
                    
                    Y = [Y; [x1;x2;x3;x4]'*Q*[x1;x2;x3;x4]+u'*R*u];
...
%--------Simulate the system performance without co-design-----------------
   K=k; [t0,y0]=ode45(@(t,x) LP_model(t,x,A,B,kBn/mB,R,k,w),[0,tf],x0);
            ...
            for x3 = -1.5:.5:1.5
                for x4 = -2:0.5:2
                    ...
                    X = [X;
						...
						(dP*(A*[x1;x2;x3;x4]+[0;kBn/mB*x3^3;0; ...
                          -kBn/mB*x3^3]+B*u))'];
...
%          k(1) k(2) k(3) k(4) mL       mB       dL   kB  dB
   ...
   [Aeq, Beq] = mat_rdc(Aeq, Beq); % extract the independent rows from Aeq
    ...
    [tc,yc] = ode45(@(t,X) LP_model_calc(t,X,A,B,kBn,Q,R,k,w), ...
		      [0, 100],[x0;zeros(4*4+4+1,1)]);
    ...
    [tcost,ycost] = ode45(@(t,X) LP_model_calc(t,X,A,B,kBn,Q,R,k,w), ...
		            [0,100],[x0;zeros(4*4+4+1,1)]);
    J1 =  ycost(end,end);
    clear tcost ycost;
...
subplot(121)
K=k;
[t,y]=ode45(@(t,X) LP_model(t,X,A,B,kBn,R,k,w),[0,tf],x0);
plot(t0,y0(:,1)+1,'b-.',t,y(:,1)+1,'r-','Linewidth',2);
%,t0,y0(:,2),t0,y0(:,3),t0,y0(:,4))
